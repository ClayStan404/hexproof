...
Download the latest Scryfall Default Cards and All Cards exports plus the
latest MTGCH Simplified Chinese data release, then build and package the
current Hexproof card database. Existing source files are never used as build
inputs.
...
if [[ "${default_url}" != https://data.scryfall.io/* ||
      ...
      "${mtgch_url}" != https://github.com/HeliumOctahelide/magic-cards-zhs/releases/download/* ]]; then
...
for source_file in default-cards.jsonl.gz all-cards.jsonl.gz \
    magic-cards-zhs-data.tar.gz upstream.json; do

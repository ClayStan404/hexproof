...
func (h *Handler) handleRoomCreate(sess *Session, env protocol.Envelope) error {
	...
	r, initialSnapshot, initialSeq, operation, err := h.hub.CreateRoomWithDeckFormat(
		...
		rc.AllowSpectators, rc.Password, sess)
	if err != nil {
		code, _ := ErrCode(err)
		if code == "" {
...
func (h *Handler) allowRoomCreate(ip string, now time.Time) bool {
	h.createRateMu.Lock()
	defer h.createRateMu.Unlock()
	for key, candidate := range h.createRates {
		if now.Sub(candidate.start) >= time.Minute {
			delete(h.createRates, key)
		}
	}
	window := h.createRates[ip]
	if window.start.IsZero() || now.Sub(window.start) >= time.Minute {
		window = createRateWindow{start: now}
	}
	if window.count >= h.config.RoomCreatesPerMinute {
		h.createRates[ip] = window
		return false
	}
	window.count++
	h.createRates[ip] = window
...
func (h *Handler) allowPasswordJoin(ip string, now time.Time) bool {
	h.joinRateMu.Lock()
	defer h.joinRateMu.Unlock()
	for key, candidate := range h.joinRates {
		if now.Sub(candidate.start) >= time.Minute {
			delete(h.joinRates, key)
		}
	}
	window := h.joinRates[ip]
	if window.start.IsZero() || now.Sub(window.start) >= time.Minute {
		window = createRateWindow{start: now}
	}
	if window.count >= h.config.PasswordJoinsPerMinute {
		h.joinRates[ip] = window
		return false
	}
	window.count++
	h.joinRates[ip] = window

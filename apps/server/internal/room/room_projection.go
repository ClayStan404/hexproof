...
// hand identities are included for the owning seat and, when the immutable
// room policy allows it, for spectators. Other private zones remain owner-only.
func (r *Room) GameSnapshot(connID string) (protocol.GameSnapshot, error) {
	...
	spectatorSeesHands := r.SpectatorsSeeHands && r.IsSpectator(connID)

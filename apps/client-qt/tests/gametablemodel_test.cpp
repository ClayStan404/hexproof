...
void TestGameTableModel::consumesSharedOwnerAndOpponentSnapshots() const
{
    const auto verifyFixture = [](const QString &name, int visibleSeat, int hiddenSeat,
        ...
        auto *hidden = qobject_cast<hexproof::client::ZoneCardModel *>(
            model.zoneModel(hiddenSeat, u"hand"_s));
        QVERIFY(visible);
        QVERIFY(hidden);
        QCOMPARE(visible->rowCount(), 7);
        QCOMPARE(hidden->rowCount(), 0);
        QCOMPARE(visible->data(visible->index(0), hexproof::client::ZoneCardModel::NameRole)
                     .toString(),
                 firstCardName);

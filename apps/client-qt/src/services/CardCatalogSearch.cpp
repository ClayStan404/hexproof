...
void CardCatalog::searchTokens(const QString &queryText)
{
    ...
    watcher->setFuture(QtConcurrent::run([databasePath, text, language]() {
...
void CardCatalog::processCardMetadataBatch()
{
    ...
    const QString databasePath = m_databasePath;
    watcher->setFuture(QtConcurrent::run([databasePath, language, batch]() {
...
void CardCatalog::enrichTokens(const QVariantList &tokens)
{
    ...
    const QString databasePath = m_databasePath;
    const QString language = m_language;
    watcher->setFuture(QtConcurrent::run([databasePath, language, tokens]() {
        QVariantList enriched;
        QSet<QString> seen;
                         ...
                         const QString &legalityFilter)
{
    ...
    watcher->setFuture(QtConcurrent::run([databasePath, text, language, normalizedType,
                                          normalizedSet, normalizedLanguage, normalizedColor,
                                          normalizedRarity, normalizedLegality]() {

...
RoomSessionState::SnapshotTransition RoomSessionState::applySnapshot(const QJsonObject &snapshot)
{
    ...
    m_allowSpectators = !m_playtest && snapshot.value(u"allowSpectators"_s).toBool();
    ...
    const bool snapshotHost = m_role == kRolePlayer && m_seatIndex >= 0 &&
                              m_seatIndex < seatArray.size() &&
    ...
    transition.returnedToRoom = previousPhase == kRoomPhaseStarted && m_phase == kRoomPhaseWaiting;

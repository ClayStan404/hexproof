...
QVariantMap validateOne(QSqlDatabase *database, bool catalogCurrent, const QVariantMap &deck)
{
    ...
    if (commanderFormat) {
        if (mainCount != 100)
            issues.append(QStringLiteral("Commander decks require exactly 100 main-deck cards."));
        if (sideboardCount != 0)
            issues.append(QStringLiteral("Commander decks cannot use a sideboard."));
        if (commanders.isEmpty() || commanders.size() > 2)
            issues.append(QStringLiteral("Commander decks require one or two commanders."));
    } else {
        if (mainCount < 60)
            issues.append(QStringLiteral("Main deck requires at least 60 cards."));
        if (sideboardCount > 15)
            issues.append(QStringLiteral("Sideboard can contain at most 15 cards."));
    }

    const bool structureValid = issues.isEmpty();
    if (!catalogCurrent || !database) {
        if (issues.isEmpty())
            issues.append(QStringLiteral("Card database required to verify deck legality."));
        return {{QStringLiteral("deckId"), deckId},
                {QStringLiteral("validationRevision"), validationRevision},
                {QStringLiteral("valid"), structureValid},
                {QStringLiteral("verified"), false},
                {QStringLiteral("status"), issues.first()},
                {QStringLiteral("issues"), issues}};
    }
    ...
    for (const QString &identity : std::as_const(identities)) {
        ...
        if (status == QStringLiteral("restricted") && copyCount > 1) {
            ...
            issues.append(QStringLiteral("%1 has %2 copies; commander formats are singleton.")
                              .arg(displayName)
                              .arg(copyCount));
        } else if (!commanderFormat && copyCount > 4 && !hasDeckLimitException(card)) {
            issues.append(QStringLiteral("%1 has %2 copies; this format allows at most four.")
                              .arg(displayName)
                              .arg(copyCount));
        }
        if (commanderFormat && !commanders.isEmpty() && commanderIdentityVerified &&
            ...
            issues.append(
                QStringLiteral("%1 is outside the commanders' color identity.").arg(displayName));
        }
    }
    ...
    return {
        ...
        {QStringLiteral("validationRevision"), validationRevision},
        {QStringLiteral("valid"), issues.isEmpty()},
        {QStringLiteral("verified"), true},
        {QStringLiteral("status"), issues.isEmpty() ? QStringLiteral("Playable") : issues.first()},
        {QStringLiteral("issues"), issues}};
}
...
void DeckLegalityService::startLatestValidation()
{
    ...
    m_watcher.setFuture(QtConcurrent::run(
        [databasePath, decks]() { return DeckLegalityService::validate(databasePath, decks); }));
}

...
- opponents and spectators receive counts and card backs for hidden zones;
...
need the current schema-v9 catalog. Settings can install a prebuilt database
...
- [`ci.yml`](.github/workflows/ci.yml) validates Linux, Windows, and macOS
  builds, tests the Go server, and runs Linux ASan/UBSan coverage.
- [`release.yml`](.github/workflows/release.yml) produces portable Windows x64,
  ...
  The Intel macOS packaging path remains supported but its automatic job is
  currently paused.
